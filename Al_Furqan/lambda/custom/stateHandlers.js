...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            var message = '<audio src="' + constants.basmalaUrl + '"/>' + constants.greeting
            var reprompt = constants.repromptGreeting
        ...
        'PlayAudio' : function () {
           ...
           if (hasSpecificIntent.call(this)) {
             serviceSpecificIntent.call(this);
           }
    ...
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            if (this.attributes['playbackFinished']) {
                this.handler.state = constants.states.START_MODE;
                message = constants.greeting;
                reprompt = constants.repromptGreeting;
            } else {
                ...
                reprompt = 'You can say yes to resume or no to play from the start.';
            }
        ...
        'PlayAudio' : function () { 
           // even if we are in play mode, if the user specifically requested something, we override.
           if (hasSpecificIntent.call(this)) {
             controller.stop.call(this);
             serviceSpecificIntent.call(this);
           }
           console.log("intent is : "+this.event.request.intent.slots.SurahQuery.value);
           controller.play.call(this) 
         },
        ...
        'AMAZON.HelpIntent' : function () {
            ...
            var message = 'You are listening to Quran Majeed. You can say, Next or Previous to navigate through the chapters. ' +
...
//TBD: return the url to play here
function hasSpecificIntent() {
   if (this.event.request.intent.slots && this.event.request.intent.slots.SurahQuery) {
     return true;
   }
   return false;
}

function serviceSpecificIntent() {
  var idx = audioAssets.searchIndex[this.event.request.intent.slots.SurahQuery.value];
  if (idx != undefined) {
    this.response.audioPlayerStop(); // stop the last playback
    this.attributes['playOrder'] = [ idx ];
    this.attributes['index'] = 0;
    this.attributes['loop'] = false;
    this.attributes['playbackIndexChanged'] = true;
    this.attributes['offsetInMilliseconds'] = 0;
    this.handler.state = constants.states.START_MODE;
    this.response.speak('Playing Surah al ' + audioData[this.attributes['playOrder'][0]].title)
  }
}
